...
[npc_op1,Kaqemeex]
	...
	if (%DruidicRitual == 3) {
		...
		%DruidicRitual = 4;
		setquest(DruidicRitual, 2); // Complete the Quest
		givexp(herblore, 250);
		return;
	}

	if (%DruidicRitual == 1 || %DruidicRitual == 2) {
	...
	if (%DruidicRitual == 0) {
		...
		switch(menu("Who are you?", "I'm in search of a quest.", "Did you build this?")) {
		case 0:
			...
			switch(menu("What about the stone circle full of dark wizards?", "So... is there anything in this for me?", "So what's so good about Guthix?", "Well, I'll be on my way now.")) {
			case 0:
				jump(DR_DarkWizards);
				switch(menu("Okay, I will try and help.", "No, that doesn't sound very interesting.")) {
				case 0:
					jump(DR_Start);
				case 1:
					jump(DR_Reject);
		...
		case 2:
			...
			switch(menu("What about the stone circle full of dark wizards?", "I'm in search of a quest.", "Well, I'll be on my way now.")) {
				case 0:
...
[label, DR_QuestExplain]
	jump(DR_DarkWizards);
	switch(menu("Okay, I will try and help.", "No, that doesn't sound very interesting.", "So... is there anything in this for me?")) {
	case 0:
		jump(DR_Start);
	case 1:
		jump(DR_Reject);
...
[label, DR_Reward]
	...
	switch(menu("Ok, I will try and help.", "No, that doesn't sound very interesting.")) {
	case 0:
		jump(DR_Start);
	case 1:
		jump(DR_Reject);
...
[label, DR_Start]
	...
	if (%DruidicRitual == 0) {
		%DruidicRitual = 1;
		setquest(DruidicRitual, 1); // Set Quest to In Progress
	}
	return;
...
if (%DruidicRitual == 2) {
	...
	if (inv_total(inv, Enchanted_Beef) > 0 || inv_total(inv, Enchanted_Bear_Meat) > 0 || inv_total(inv, Enchanted_Rat_Meat) > 0 || inv_total(inv, Enchanted_Chicken) > 0) {
		...
		%DruidicRitual = 3;
		message_game("Sanfew grunts");
		return;
	} else {
		chatplayer("No, not yet...");
		chatnpc("Well let me know when you do young 'un.");
		switch(menu("What was I meant to be doing again?", "I'll get on with it.")) {
		case 0:
...
if (%DruidicRitual == 1) {
	...
	switch(menu("I've been sent to help purify the Varrock stone circle.", "Actually, I don't need to speak to you.")) {
	case 0:
...
[label, Sanfew_Cauldron]
	switch(menu("Where can I find this cauldron?", "Ok, I will do that then.")) {
	case 0:
...
[npc_useItem,ThunderCauldron] (int $item)

if ($item == Raw_Beef) {
	message_game("You dip the raw beef in the cauldron.");
	inv_del(inv, Raw_Beef, 1);
	inv_get(inv, Enchanted_Beef, 1);
} else if ($item == Raw_Bear_Meat) {
	message_game("You dip the raw bear meat in the cauldron.");
	inv_del(inv, Raw_Bear_Meat, 1);
	inv_get(inv, Enchanted_Bear_Meat, 1);
} else if ($item == Raw_Rat_Meat) {
	message_game("You dip the raw rat meat in the cauldron.");
	inv_del(inv, Raw_Rat_Meat, 1);
	inv_get(inv, Enchanted_Rat_Meat, 1);
} else if ($item == Raw_Chicken) {
	message_game("You dip the raw chicken in the cauldron.");
	inv_del(inv, Raw_Chicken, 1);
	inv_get(inv, Enchanted_Chicken, 1);
} else {
	not_interesting();
}
